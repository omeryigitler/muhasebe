...
const FAQItem: React.FC<FAQItemProps> = ({ index, question, answer, isOpen, theme, onClick }) => {
  ...
  return (
    <div
      ...
      className={cn(
        'mb-4 md:mb-6 rounded-[20px] md:rounded-3xl border-2 transition-all duration-300 overflow-hidden',
        isActive
          ? `border-deep-ink ${theme.bg} shadow-[5px_5px_0px_#101114] md:shadow-[6px_6px_0px_#101114] -translate-y-1`
    ...
    >
      <button
        ...
        className="w-full flex items-center justify-between p-4 md:p-8 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-deep-ink focus-visible:ring-inset"
      >
        <div className="flex items-start md:items-center gap-3 md:gap-6 min-w-0">
          <span className={cn('font-mono text-sm md:text-2xl pt-1 md:pt-0 transition-colors duration-300 flex-shrink-0', isActive ? theme.numActive : 'text-deep-ink/40')}>
            {num}
          </span>
          <h3 className={cn('font-display text-xl md:text-3xl pr-2 md:pr-4 leading-tight transition-colors duration-300', isActive ? theme.text : 'text-deep-ink')}>
            {question}
          </h3>
        </div>
        <div className={cn(
          'flex-shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-full border-2 flex items-center justify-center transition-all duration-500',
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="px-4 md:px-8 pb-5 md:pb-8 pt-1 md:pt-2 md:pl-[88px]">
              <p className={cn('font-mono text-sm md:text-lg leading-relaxed max-w-3xl opacity-90', theme.text)}>
                {answer}
              </p>
            </div>
          </motion.div>
...
export const FAQ = () => {
  ...
  return (
    <section className="py-20 md:py-32 px-4 md:px-8 bg-warm-paper text-deep-ink relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 relative z-10">
        <div className="lg:col-span-5">
          <div className="lg:sticky lg:top-32">
            <h2 className="font-display text-4xl md:text-7xl text-deep-ink tracking-tight mb-5 md:mb-8">{t('faq.title')}</h2>
            <p className="font-mono text-base md:text-xl opacity-70 max-w-md leading-relaxed">{t('faq.desc')}</p>
          </div>
        </div>
