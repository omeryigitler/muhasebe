...
export const Hero = () => {
  ...
  useGSAP(() => {
    ...
    const runDemoKeys = (timeline: gsap.core.Timeline) => {
      [
        { t: 1.0, k: '4' },
        { t: 1.1, k: '8' },
        { t: 1.2, k: '2' },
    ...
    mm.add('(min-width: 768px)', () => {
      ...
      if (window.innerWidth > 1024) {
        ...
        startX = window.innerWidth / 2 - (rect.left + rect.width / 2);
      }

      if (reduceMotion) {
        gsap.set(calcStageRef.current, { clearProps: 'all' });
        gsap.set(wrapper, { x: 0, y: 0, scale: 1, rotationX: 0, rotationY: 0, rotationZ: 0, opacity: 1 });
      ...
      setIsInteractive(false);
      gsap.set(calcStageRef.current, { clearProps: 'all' });
      gsap.set(wrapper, { scale: 0.8, rotationY: 15, rotationX: 10, y: 50, x: startX, opacity: 0 });
      gsap.set(heroCopyRef.current, { autoAlpha: 1, y: 0 });
      ...
      const intro = gsap.timeline({ onComplete: () => setIsInteractive(true) });
      introTimelineRef.current = intro;
      intro.to(wrapper, { scale: 1.08, rotationY: 5, rotationX: 5, y: 0, opacity: 1, duration: 1.2, ease: 'power3.out' }, 0);
      runDemoKeys(intro);
      ...
      intro.to(wrapper, { x: 0, rotationY: 0, rotationX: 0, scale: 1, duration: 1.2, ease: 'power3.inOut' }, 5.3);
      ...
      storySteps.forEach((_, index) => {
        ...
        storyTimeline.to(wrapper, { y: index % 2 === 0 ? -8 : 8, rotationZ: index % 2 === 0 ? -1.2 : 1.2, duration: 0.55, ease: 'power2.inOut' }, start + 0.1);
      ...
      storyTimeline.to(wrapper, { y: 0, rotationZ: 0, duration: 0.5, ease: 'power2.out' }, 5.7);
        ...
        <div className="max-w-7xl h-full md:h-auto w-full mx-auto relative md:grid md:grid-cols-1 lg:grid-cols-2 md:gap-4 lg:gap-16 md:items-center z-10">
          <div className="absolute inset-x-0 bottom-0 h-[46%] md:relative md:h-auto md:min-h-[330px] lg:min-h-[560px] md:order-1 lg:pr-8 z-20">
            <div ref={heroCopyRef} className="absolute inset-0 flex flex-col justify-center md:justify-center gap-2.5 sm:gap-5 lg:gap-6">
              ...
              <h1 ref={headlineRef} className="font-display text-[2.15rem] leading-[0.86] sm:text-6xl md:text-7xl lg:text-[6rem] lg:leading-[0.9] flex flex-col gap-1 sm:gap-2 font-black tracking-tight">
              ...
              <p ref={subtextRef} className="text-[12px] sm:text-base md:text-xl text-warm-paper/70 max-w-md leading-relaxed">{t('hero.desc')}</p>
