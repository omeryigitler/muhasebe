...
export const Stats = () => {
  ...
  return (
    <section className="py-16 md:py-28 px-4 md:px-8 bg-deep-ink border-t border-white/10 text-warm-paper">
      <div className="max-w-7xl mx-auto">
        <p className="font-mono text-[10px] uppercase tracking-[0.32em] text-white/30 mb-6 md:mb-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 border-t border-l border-white/10">
          {principles.map((item, index) => (
            <article key={item.title} className="group relative min-h-[175px] sm:min-h-[210px] lg:min-h-[270px] p-4 sm:p-6 md:p-8 border-r border-b border-white/10 flex flex-col justify-between overflow-hidden isolate">
              <div className={`absolute inset-0 z-0 ${item.layer} translate-y-[calc(100%_-_5px)] lg:translate-y-full lg:group-hover:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(.2,.8,.2,1)]`} />

              <div className="relative z-10 flex items-start justify-between gap-3">
                <span className={`font-display font-bold text-5xl sm:text-6xl md:text-8xl leading-none text-acid-lime transition-[transform,color] duration-500 lg:group-hover:scale-110 lg:group-hover:rotate-[4deg] origin-left ${item.hoverText}`}>
                  {item.symbol}
                </span>
                <span className={`font-mono text-[8px] md:text-[9px] tracking-[0.2em] md:tracking-[0.24em] text-white/25 transition-colors duration-300 ${item.hoverMuted}`}>
                  {String(index + 1).padStart(2, '0')}
                </span>
              </div>

              <div className={`relative z-10 transition-colors duration-300 ${item.hoverText}`}>
                <h3 className="font-display text-2xl md:text-[2rem] mb-2 md:mb-3 tracking-tight leading-none">{item.title}</h3>
                <p className={`text-[12px] sm:text-sm md:text-base text-white/45 leading-snug md:leading-relaxed transition-colors duration-300 ${item.hoverMuted}`}>
                  {item.description}
                </p>
              </div>
            </article>
